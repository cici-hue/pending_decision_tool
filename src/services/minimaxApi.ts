    ...
    - **提取逻辑**（非常重要）：
      ...
      5. **匹配示例**：
         ...
         **示例 C**：范围对多个具体值（**不要合并**）
         ...
         - 结果：三条独立记录，分别列出，不要加总
    - **疵点描述（极其重要）**：
      - **只使用 Comments 列的内容，绝对不要包含 Description 列的内容**
      - **Defect Summary 表格有四列**：Code / Description / Comments / No. of Defects
      - **Description 列**：是类别描述（如 "Measurement/ height: longer than default"），**绝对不要提取这个**
      - **Comments 列**：是具体缺陷描述（如 "armhole height plus 1cm"），**只提取这个**
      - **重要**：不要把 Description 和 Comments 的内容拼接在一起
      - **错误示例**："Measurement/ height: longer than default armhole height plus 1cm"（包含了Description，这是错的）
      - **正确示例**："armhole height plus 1cm"（只用Comments）
    ...
    const response = await fetch(DOUBAO_API_URL, {
      ...
      body: JSON.stringify({
        ...
        messages: [
          {
            role: 'user',
            content: PDF_EXTRACTION_PROMPT + pdfText
          }
        ]
      })
    })
